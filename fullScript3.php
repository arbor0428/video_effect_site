			 ...
			 if(index2==3){
				 ...
				 if(i==1){
					...
					function counterUp(){
						...
						countOne2= 40+countOne*0.05
						if(countOne2<40)	countOne2=40
						if(countOne2 <= 80){
							$(".cirWrap circle").attr("r",countOne2);
						}else{
							$(".cirWrap circle").attr("r",80);
						}
						if(countOne >= 940){
							clearInterval(stop)
							$(".txtCir p").delay(300).fadeIn();
							$(".cirWrap text").delay(300).fadeIn();
					...
					$(".cirWrap").attr('class',"cirWrap motion<?=$moTxt?> opacity");
					gsap.config({
					...
					var stop2 = setTimeout(function() { 
						...
						$(".cirWrap").attr('class',"cirWrap motion2<?=$moTxt?> opacity");
	...
	function index5Effect	(){
		...
		$('.sec05 .lineBar').delay(800).animate({'height':'204px'},800,function(){
			$('.barTxtBox').stop().animate({'opacity':1},1000,function(){
				$('.contactBtn').stop().animate({'opacity':1},1000,function(){
					$('.sec05Bot').addClass('active');
				});
			});
		 });
	}
