...
.pageNum {margin: 80px 0 100px 0; text-align:center;}
.pageNum a {display: inline-block; border-radius: 50%;width:32px; height:32px; line-height:32px; margin:0 5px;}
.pageNum .this,.pageNum a:hover { background-color :#4169E2; color:#fff;}
.pageNum .direction:hover {background:none; color:inherit;}
.pageNum .lnr {
	font-weight: bold;
}
...
if($total_record != '0'){
	if($total_record > $record_count){
		...
		if($total_page > 1 && ($record_start !=0 )) {
			...
			echo ("<a href=javascript:pageNum('$fName','$pre_page_start'); class='direction'><span class='lnr lnr-chevron-left'></a> ");
		}else{
			echo ("<a href=javascript:pageNum('$fName','$pre_page_start'); class='direction'><span class='lnr lnr-chevron-left'></a> ");
		}
		...
		if($total_page > 1 && ($record_start != ($total_page * $record_count - $record_count))) {
			...
			echo ("<a href=javascript:pageNum('$fName','$next_page_start'); class='direction'><span class='lnr lnr-chevron-right'></a> ");
		}else{
